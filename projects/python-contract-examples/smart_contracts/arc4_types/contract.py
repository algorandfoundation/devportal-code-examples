...
class Arc4Types(ARC4Contract):
    ...
    def arc4_address(self, address: arc4.Address) -> arc4.Address:
        underlying_bytes = address.bytes # This will return the underlying bytes of the address.

        account = address.native # This will return the account type of the given address.
        bal = account.balance
...
class Arc4StaticArray(ARC4Contract):
    ...
    def arc4_static_array(self) -> None:
        ...
        """
        static_uint32_array = arc4.StaticArray(arc4.UInt32(1), arc4.UInt32(10), arc4.UInt32(255), arc4.UInt32(128))
    ...
    def complete_todo(self, task: arc4.String) -> None:
        ...
        for index in urange(self.todos.value.length):
            if self.todos.value[index].task == task:
                self.todos.value[index].completed = arc4.Bool(True)
                break
